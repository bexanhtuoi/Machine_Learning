...
#                            y = np.array([[0], [1], [0]])
class BSVM: # Binary Support Vector Machines
    def __init__(self, learning_rate=0.001, n_iters=1000, val_rate=0.2, optimizer="GD", C=1.0, lamda=0.9, alpha=0.9, beta_1=0.992, beta_2=0.999, gamma=0.9, epsilon=1e-8, mini_batch=None, decay=None):
    ...
    def cost_function(self, y, y_predicted):
      N = X.shape[0]
      z = 1 - (y * y_predicted)
      return 1/N * np.sum(np.maximum(0, z)) + (1/2 * self.C * np.sum(self.weights**2))

    def Derivative(self, X, y, y_predicted):
        N = X.shape[0]
        z = 1 - (y * y_predicted)
        margin = np.maximum(0, z)
        loss = np.where(margin > 0, 1, 0 )
        dw = -(1 / N) * np.dot(X.T, loss * y) + self.C * self.weights
        db = -(1 / N) * np.sum(loss * y)
        return dw, db
